...
export const getVolunteers = async (req: Request, res: Response) => {
  ...
  } catch { res.status(500).json({ success: false, error: "Failed to fetch volunteers" }); }
};

export const updateVolunteerStatus = async (req: Request, res: Response) => {
  try {
    const volunteer = await prisma.volunteer.update({ where: { id: req.params.id as string }, data: req.body, include: { member: true } });
    res.json({ success: true, data: volunteer });
  } catch { res.status(500).json({ success: false, error: "Failed to update volunteer" }); }
};
